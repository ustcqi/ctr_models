...
class DIN(object):
  ...
  def build_model(self):
    ...
    with tf.name_scope("attention"):
      self.attention_output = self.attention(self.target_ad_embedding, self.hist_click_seq_embeddings, self.hist_click_length)
    ...
    with tf.name_scope("prediction_net"):
      self.pred_input = tf.concat([self.user_flatten, self.ad_flatten, self.cross_flatten, self.attention_output], axis=1)
      self.pred_input = tf.reshape(self.pred_input, [-1, (FLAGS.user_input_length + FLAGS.ad_input_length + FLAGS.cross_input_length + 1) * FLAGS.embedding_dim])

      self.pred_w1 = tf.Variable(tf.glorot_uniform_initializer()( ((FLAGS.user_input_length + FLAGS.ad_input_length + FLAGS.cross_input_length + 1) * FLAGS.embedding_dim, FLAGS.pred_layer1_units)), name="pred_w1")
