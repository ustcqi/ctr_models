  ...
  for part in eval_part_list:
    ...
    while True:
      try:
        ...
        pred_score, loss = sess.run([dnn.pred_score, dnn.loss], feed_dict={dnn.user_input : batch_user_feature, dnn.cross_input : batch_topic_feature, dnn.ad_input : batch_ad_feature, dnn.label : batch_label})
  ...
  with tf.Session(config=config) as sess:
    ...
    for epoch in range(train_params["train_epoch"]):
      for part in train_part_list:
        ...
        while True:
          try:
            # (batch_user_feature, batch_ad_feature, batch_label) = sess.run([user_features, ad_features, labels])
            (batch_user_feature, batch_topic_feature, batch_ad_feature, batch_label) = sess.run([user_features, topic_features, ad_features, labels])
            sess.run(dnn.train_step, feed_dict={dnn.user_input : batch_user_feature, dnn.cross_input : batch_topic_feature, dnn.ad_input : batch_ad_feature, dnn.label : batch_label})
          except tf.errors.OutOfRangeError:
            break
        if i % 10 == 0:
          auc, avg_loss = eval(sess, dnn, eval_part_list, eval_params)
