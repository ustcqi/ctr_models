...
class DataIterator(object):
  ...
  def decode_csv(self, record):
    ...
    # label
    ctr_labels = tf.string_to_number(records.values[4:5], tf.float32)
    ctr_labels = tf.reshape(ctr_labels, [-1])
    cvr_labels = tf.string_to_number(records.values[4:5], tf.float32)
    cvr_labels = tf.reshape(cvr_labels, [-1])
    ...
    return user_input, cross_input, ad_input, hist_click_seq, hist_click_length, target_ad, ctr_labels, cvr_labels
