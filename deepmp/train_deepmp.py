...
def eval(sess, deepmp, eval_part_list, eval_params):
  auc = 0.0  
  avg_loss = 0.0
  scores = np.zeros((1, 1))
  losses = np.zeros((1, 1))
  ...
  for part in eval_part_list:
    ...
    while True:
      try:
        ...
        pred_score, match_score, loss = sess.run([deepmp.pred_score, deepmp.match_score, deepmp.loss], feed_dict={deepmp.user_input : batch_user_feature, deepmp.cross_input : batch_topic_feature, deepmp.ad_input : batch_ad_feature, deepmp.label : batch_label})
    ...
    for epoch in range(train_params["train_epoch"]):
      for part in train_part_list:
        ...
        while True:
          try:
            (batch_user_feature, batch_topic_feature, batch_ad_feature, batch_label) = sess.run([user_features, topic_features, ad_features, labels])
            sess.run(deepmp.train_op, feed_dict={deepmp.user_input : batch_user_feature, deepmp.cross_input : batch_topic_feature, deepmp.ad_input : batch_ad_feature, deepmp.label : batch_label})
          except tf.errors.OutOfRangeError:
            break
        if i % 10 == 0:
          auc, avg_loss = eval(sess, deepmp, eval_part_list, eval_params)
          logging.info("epoch %d, auc %.4f, loss %.4f" % (epoch, auc, avg_loss[0]))
        i += 1
    auc, avg_loss = eval(sess, deepmp, eval_part_list, eval_params)
    logging.info("epoch %d, auc %.4f, loss %.4f" % (epoch, auc, avg_loss[0]))

    # saver.save(sess, "./model/ctr_model.ckpt")
    # dump_model(sess, deepmp)
